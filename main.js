...
function renderProducts(list) {
  ...
  list.forEach(item => {
    ...
    if (count > 0) {
      ...
      countEl.className = "count-number"; countEl.textContent = count;
...
categories.querySelectorAll("div").forEach(cat => {
  cat.onclick = () => {
    ...
    // возврат на главную через 0.25s
    setTimeout(() => {
...
function updateCartUI() {
  const c = cart.reduce((s,i)=>s.count ? s.count : i.count,0);
  const t = cart.reduce((s,i)=>s.count*i.product.price,0);
