...
function renderProducts(list=products){
  ...
  list.forEach(p=>{
    ...
    const img=new Image(); img.src=p.image; img.alt=p.name;
...
function addToCart(product){const cartItem=cart.find(i=>i.product.id===product.id); if(cartItem) cartItem.count++; else cart.push({product,count:1}); renderProducts(inCartScreen?cart.map(i=>i.product):products);}
function removeFromCart(product){const cartItem=cart.find(i=>i.product.id===product.id); if(!cartItem) return; cartItem.count--; if(cartItem.count===0) cart=cart.filter(i=>i.product.id!==product.id); renderProducts(inCartScreen?cart.map(i=>i.product):products);}
...
searchInput.addEventListener("input",e=>{const v=e.target.value.toLowerCase(); renderProducts(products.filter(p=>p.name.toLowerCase().includes(v)));});
menuIcon.onclick=()=>categories.classList.toggle("show");
categories.querySelectorAll("div").forEach(cat=>{cat.onclick=()=>{inCartScreen=false; renderProducts(cat.dataset.category==="Главная"?products:products.filter(p=>p.category===cat.dataset.category)); categories.classList.remove("show");};});
...
function flyToCart(img){const fly=img.cloneNode(); fly.className="fly-to-cart"; const rect=img.getBoundingClientRect(); fly.style.left=rect.left+"px"; fly.style.top=rect.top+"px"; document.body.appendChild(fly); setTimeout(()=>{const cartRect=cartButton.getBoundingClientRect(); fly.style.transform=`translate(${cartRect.left-rect.left}px,${cartRect.top-rect.top}px) scale(0.1)`; fly.style.opacity="0";},10); setTimeout(()=>fly.remove(),610);}
