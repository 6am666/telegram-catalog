...
function updateOrderSum(){
  ...
  switch(deliverySelect.value){
    case "СДЭК": d=450; break;
    case "Почта России": d=550; break;
    case "Яндекс.Доставка": d=400; break;
    default: d=0;
  }
  orderSum.textContent = `Итоговая сумма: ${total+d} ₽`;
  deliveryInfo.textContent = deliverySelect.value==="Самовывоз" ? "Самовывоз — СПб, Русановская 18к8": "";
}
deliverySelect.addEventListener("change", updateOrderSum);

// ================== КНОПКА ОПЛАТЫ ==================
orderForm.onsubmit = async e => {
  e.preventDefault();
  if(isSubmitting || !cart.length) return alert("Корзина пуста или данные заказа отсутствуют");
  isSubmitting=true;

  let total = cart.reduce((s,i)=>s+i.count*i.product.price,0);
  switch(deliverySelect.value){
    case "СДЭК": total+=450; break;
    case "Почта России": total+=550; break;
    case "Яндекс.Доставка": total+=400; break;
  }
  ...
  try{
    ...
    if(!json.payment_url) throw new Error("Ошибка оплаты");
