...
function renderProducts(list) {
  ...
  list.forEach(item => {
    // если корзина, item уже {product,count}
    ...
    if (cart.find(i => i.product.id === product.id)) {
      ...
      minus.textContent = "–";
      minus.onclick = e => { e.stopPropagation(); removeFromCart(product); };
      const countEl = document.createElement("div");
      countEl.className = "count-number";
      countEl.textContent = itemInCart.count;
      const plus = document.createElement("button");
      plus.textContent = "+";
      plus.onclick = e => { e.stopPropagation(); addToCart(product); };
      controls.append(minus, countEl, plus);
    } else {
      const btn = document.createElement("button");
      btn.textContent = "В корзину";
      btn.onclick = e => { e.stopPropagation(); addToCart(product); };
...
// ================== ПЕРЕКЛЮЧЕНИЕ МЕЖДУ КОРЗИНОЙ И ГЛАВНОЙ ==================
cartButton.onclick = () => {
  inCartScreen = true;
...
mainTitle.onclick = () => {
  inCartScreen = false;
  document.body.classList.remove("cart-mode", "category-mode");
...
categories.querySelectorAll("div").forEach(cat => {
  cat.onclick = () => {
    ...
    // плавный возврат на главную через 0.5s
    setTimeout(() => {
      document.body.classList.remove("category-mode");
      pageWrapper.style.transform = "translateX(0)";
    }, 500);
...
searchInput.oninput = () => {
  ...
  renderProducts(getCurrentList().filter(p => p.product ? p.product.name.toLowerCase().includes(val) : p.name.toLowerCase().includes(val)));
};
