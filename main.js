...
function updateOrderSum(){
  ...
  let d = 0;
  if(deliverySelectEl.value==="СДЭК") d=450;
  if(deliverySelectEl.value==="Почта России") d=550;
  if(deliverySelectEl.value==="Яндекс.Доставка") d=400;
  orderSumEl.textContent = `Итоговая сумма: ${total+d} ₽`;
  deliveryInfoEl.textContent = deliverySelectEl.value==="Самовывоз"
    ? "Самовывоз: СПб, Русановская 18к8" : "";
}
...
orderForm.onsubmit = async e => {
  e.preventDefault();
  if(isSubmitting || !cart.length) return alert("Корзина пуста");
  isSubmitting = true;

  let deliveryCost = 0;
  if(deliverySelectEl.value==="СДЭК") deliveryCost=450;
  if(deliverySelectEl.value==="Почта России") deliveryCost=550;
  if(deliverySelectEl.value==="Яндекс.Доставка") deliveryCost=400;
  ...
  try {
    ...
    if(!json.payment_url) throw new Error("no url");
